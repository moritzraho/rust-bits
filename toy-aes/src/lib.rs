...
mod tests {
    ...
    fn known_answer_fips197_aes128() {
        ...
        let mut pt = [0u8; 16];
        for i in 0..16 {
            pt[i] = dmatrix[i / 4][i % 4];
        }
        assert_eq!(pt, plaintext);
    }

    // --- sanity / edge behavior ---------------------------------------
    ...
    #[test]
    #[should_panic(expected = "text must be exactly")]
    fn from_text_wrong_length_panics() {
        // must be exactly 16 bytes
        Block::from_text("too short");
    }

    #[test]
    fn to_string_from_text_is_identity() {
        let s = "Sixteen bytes!!!";
        assert_eq!(s.len(), 16);
        let block = Block::from_text(s);
